...
df_countries=df[df.iso_code!='0']
df_countries=df_countries[df_countries.year==2018]
#print(df_major.sample(30))
df_major_year=df_major[df_major.year==2018]

yearmks = [*range(1950,2020,10)]

fig_mloa=px.line(mloa_df, x='Date',y='co2',title='Mauna Loa CO2 Measurements')
fig_choro=px.choropleth(df_countries,locations='iso_code',color='co2_per_capita',color_continuous_scale=px.colors.sequential.Plasma)
...
app.layout=html.Div(className='top-level',children=[
    ...
    html.Div(className='bigrow',children=[
        ...
        html.Div(className='main-content',children=[
            ...
            dcc.Graph(id='tsa_plot',
                figure=px.line(df_major,x='year',y='co2',title='CO2 Production',color='country'))
...
def update_plot(inval,year_range, indiv_country, useyear):
    ...
    df_country = df_countries[(df_countries.country==indiv_country)&(df_countries.year==useyear)]
    ...
    fig_bar=px.bar(df_major_year,x='country',y=['coal_co2','gas_co2','oil_co2'],title='CO2 Makeup')

    fig_choro=px.choropleth(df_countries,locations='iso_code',color=inval,
                            color_continuous_scale=px.colors.sequential.Plasma)

    figure=px.line(newdf,x='year',y=inval,title='CO2 Production',color='country')
    return (fig_choro,figure, fig_bar)
